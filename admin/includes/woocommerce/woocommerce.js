...
(function($) {
    ...
    function initIntegrationToggle() {
        $('#woocommerce_integration_toggle').on('change', function() {
            ...
            if (isEnabled) {
                if (confirm(swsib_woocommerce.confirm_enable)) {
                    toggleWooCommerceIntegration(true);
                } else {
                    $(this).prop('checked', false);
                    isProcessingToggle = false;
                }
            } else {
                if (confirm(swsib_woocommerce.confirm_disable)) {
                    toggleWooCommerceIntegration(false);
                } else {
    ...
    /**
     * Toggle WooCommerce integration via AJAX
     */
    function toggleWooCommerceIntegration(enable) {
        $('#woocommerce_toggle_result').html('<span class="loading">' + swsib_woocommerce.testing_message + '</span>');
        
        $.ajax({
            url: swsib_woocommerce.ajaxurl,
            type: 'POST',
            data: {
                action: 'swsib_toggle_woocommerce_integration',
                nonce: swsib_woocommerce.nonce,
                enable: enable
            },
            success: function(response) {
                if (response.success) {
                    $('#woocommerce_toggle_result').html('<span class="success">' + response.data.message + '</span>');
                    $('#integration_enabled_field').val(enable ? '1' : '0');
                    ...
                    $('#woocommerce_toggle_result').html('<span class="error">' + response.data.message + '</span>');
                    $('#woocommerce_integration_toggle').prop('checked', !enable);
                    
                    if (response.data.conflict) {
                        alert('PE Subscription integration is currently active. Please disable it first.');
                    }
            ...
            error: function() {
                $('#woocommerce_toggle_result').html('<span class="error">Error occurred while updating integration status.</span>');
                $('#woocommerce_integration_toggle').prop('checked', !enable);
                isProcessingToggle = false;
            }
        });
