...
class SwiftSpeed_Siberian_DB_Backup {
    ...
    public function enqueue_scripts($hook) {
        ...
        wp_enqueue_style(
            'swsib-db-backup-css',
            SWSIB_PLUGIN_URL . 'admin/includes/advanced-features/backup/db-backup.css',
            array(),
            SWSIB_VERSION . '.' . time()
        );
        wp_enqueue_script(
            'swsib-db-backup-js',
            SWSIB_PLUGIN_URL . 'admin/includes/advanced-features/backup/db-backup.js',
            array('jquery'),
            SWSIB_VERSION . '.' . time(),
            true
        );
                                ...
                                <?php foreach ($backups as $backup): ?>
                                    <tr>
                                        ...
                                        <td class="backup-actions">
                                            <a href="<?php echo esc_url($backup['url']); ?>" class="button download-backup" data-file="<?php echo esc_attr($backup['file']); ?>">
        ...
        foreach ($files as $file) {
            ...
            $backups[] = array(
                ...
                'url'  => add_query_arg(array(
                    ...
                    'file'    => $filename,
                    '_wpnonce'=> wp_create_nonce('swsib_download_backup')
    ...
    private function process_next_table($status) {
        ...
        $status['message'] = sprintf(__('Backing up table: %s (%d of %d)', 'swiftspeed-siberian'),
            $table, $status['processed_tables'] + 1, $status['total_tables']);
        update_option('swsib_current_backup', $status);
    ...
    // Export table structure and data to an SQL file
    private function export_table($status, $table) {
        ...
        if (!$file) {
            return new WP_Error('file_open', sprintf(__('Could not open output file for table: %s', 'swiftspeed-siberian'), $table));
        }
        ...
        if (!$create_table_result) {
            fclose($file);
            return new WP_Error('query_error', sprintf(__('Error getting table structure for %s: %s', 'swiftspeed-siberian'), $table, $conn->error));
        }
        ...
        if ($row_count > 0) {
            ...
            if ($row_count <= 500) {
                ...
                if (!$data_result) {
                    fclose($file);
                    return new WP_Error('query_error', sprintf(__('Error exporting data for %s: %s', 'swiftspeed-siberian'), $table, $conn->error));
                }
                ...
                for ($batch = 0; $batch < $batches; $batch++) {
                    ...
                    $data_query = "SELECT * FROM `{$table}` LIMIT {$offset}, {$batch_size}";
                    $data_result = $conn->query($data_query);
                    if (!$data_result) {
                        fclose($file);
                        return new WP_Error('query_error', sprintf(__('Error exporting data for %s: %s', 'swiftspeed-siberian'), $table, $conn->error));
                    ...
                    if ($batch % 5 === 0 || $batch === $batches - 1) {
                        $percent_complete = (($offset + $batch_size) / $row_count) * 100;
                        if ($percent_complete > 100) $percent_complete = 100;
                        $status['message'] = sprintf(__('Backing up table: %s (%d/%d tables, %d%% of rows)', 'swiftspeed-siberian'),
                            $table, $status['processed_tables'] + 1, $status['total_tables'], floor($percent_complete));
                        if ($status['total_tables'] > 0) {
    ...
    // Create a final ZIP archive with a single combined SQL file and a README
    private function create_final_archive($status) {
        ...
        // Combine all SQL files into one file
        ...
        // Create ZIP archive with the combined SQL file and a README file
        $zip = new ZipArchive();
        ...
        $this->log_message('Backup process completed successfully. Archive: ' . $status['file'] . ' (' . $status['size'] . ')');
    }
    ...
    // AJAX handler for checking backup progress (optional)
    ...
    public function ajax_download_backup() {
        if (!isset($_GET['_wpnonce']) || !wp_verify_nonce($_GET['_wpnonce'], 'swsib_download_backup')) {
        ...
        if (empty($file)) {
            wp_die(__('No file specified', 'swiftspeed-siberian'));
        }
        if (pathinfo($file, PATHINFO_EXTENSION) !== 'zip' || !file_exists($this->backup_dir . $file)) {
    ...
    public function ajax_delete_backup() {
        ...
        if (empty($file)) {
            wp_send_json_error(array('message' => __('No file specified', 'swiftspeed-siberian')));
        }
        if (pathinfo($file, PATHINFO_EXTENSION) !== 'zip' || !file_exists($this->backup_dir . $file)) {
