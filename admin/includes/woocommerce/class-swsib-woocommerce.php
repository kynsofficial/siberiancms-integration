...
class SwiftSpeed_Siberian_WooCommerce
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_swsib_toggle_woocommerce_integration', array($this, 'ajax_toggle_integration'));
        ...
        // Order data processing hooks - only add these if integration is enabled
    ...
    public function is_integration_enabled() {
        return isset($this->options['woocommerce']['integration_enabled']) && 
               filter_var($this->options['woocommerce']['integration_enabled'], FILTER_VALIDATE_BOOLEAN);
    }
    ...
    public function ajax_toggle_integration() {
        ...
        $this->log_message("WooCommerce integration " . ($enable ? "enabled" : "disabled"));
    ...
    public function process_form_submission()
    {
        ...
        // Use a unique nonce field name to avoid duplicate IDs.
        if (!isset($_POST['_wpnonce_swsib_woocommerce']) || !wp_verify_nonce($_POST['_wpnonce_swsib_woocommerce'], 'swsib_woocommerce_nonce')) {
        ...
        if (isset($_POST['swsib_options']['woocommerce'])) {
            ...
            if (!isset($woocommerce['integration_enabled']) && isset($current_opts['integration_enabled'])) {
                $woocommerce['integration_enabled'] = $current_opts['integration_enabled'];
            }
            ...
            foreach ($shortcode_fields as $field) {
                if (isset($woocommerce[$field])) {
                    // If it starts with a shortcode character, do not escape.
    ...
    {
        // Only load on your plugin admin page(s).
        if (strpos($hook, 'swsib-integration') === false) {
            return;
        }

        // Enqueue the jQuery UI Sortable library to ensure drag-and-drop works.
        ...
        // Enqueue your custom script, making sure jQuery UI Sortable is a dependency.
        wp_enqueue_script(
            'swsib-woocommerce-js',
            SWSIB_PLUGIN_URL . 'admin/includes/woocommerce/woocommerce.js',
            array('jquery', 'jquery-ui-sortable'), // Include jquery-ui-sortable here
            SWSIB_VERSION,
            true
        );

        // Get options for detecting conflicts
        $options = get_option('swsib_options', array());
        ...
        $pe_enabled = isset($options['subscription']['integration_enabled']) ? 
                     filter_var($options['subscription']['integration_enabled'], FILTER_VALIDATE_BOOLEAN) : 
                     false;
        $integration_conflict = $woo_enabled && $pe_enabled;
        ...
        $localized_data = array(
            ...
            'integration_enabled' => $woo_enabled,
            'pe_integration_enabled' => $pe_enabled,
            'integration_conflict' => $integration_conflict,
            'confirm_enable'    => __('Are you sure you want to enable WooCommerce integration?', 'swiftspeed-siberian'),
            'confirm_disable'   => __('Are you sure you want to disable WooCommerce integration? This will affect any existing subscriptions.', 'swiftspeed-siberian')
        );
    ...
    public function save_custom_data_to_order_items($item, $cart_item_key, $values, $order)
    {
        if (isset($values['custom_data'])) {
            ...
            // Save as consolidated meta data.
    ...
    public function display_settings()
    {
        ...
        $integration_enabled = isset($woo_options['integration_enabled']) ? 
                              filter_var($woo_options['integration_enabled'], FILTER_VALIDATE_BOOLEAN) : 
                              false;

        // Check for PE Subscription integration status
        $pe_enabled = isset($this->options['subscription']['integration_enabled']) ? 
                     filter_var($this->options['subscription']['integration_enabled'], FILTER_VALIDATE_BOOLEAN) : 
                     false;

        // Check for integration conflict
        $integration_conflict = $integration_enabled && $pe_enabled;
        ...
        <?php if ($integration_conflict): ?>
        <div class="swsib-notice error">
            <p><strong><?php _e('Integration Conflict Detected', 'swiftspeed-siberian'); ?></strong></p>
            <p><?php _e('Both WooCommerce and PE Subscription integrations are currently active. This can cause conflicts. Please disable one of them.', 'swiftspeed-siberian'); ?></p>
        </div>
        <?php endif; ?>

        <?php if ($pe_enabled && !$integration_enabled): ?>
        <div class="swsib-notice warning">
            <p><strong><?php _e('PE Subscription Integration Active', 'swiftspeed-siberian'); ?></strong></p>
            <p><?php _e('The PE Subscription integration is currently active. You need to disable it before enabling WooCommerce integration.', 'swiftspeed-siberian'); ?></p>
            <p><a href="<?php echo admin_url('admin.php?page=swsib-integration&tab_id=subscription'); ?>" class="button"><?php _e('Use Native Subscription', 'swiftspeed-siberian'); ?></a></p>
        </div>
        <?php endif; ?>
        ...
        <div class="swsib-section">
            ...
            <div class="swsib-field">
                <label class="swsib-toggle-label">
                    <input type="checkbox" 
                           id="woocommerce_integration_toggle" 
                           <?php checked($integration_enabled); ?> 
                           <?php disabled($pe_enabled); ?>>
                ...
                <p class="swsib-field-note">
                    <?php _e('Sell PE Subscription with WooCommerce Subscription. You cannot enable this feature if you are already using Sell With WooCommerce.', 'swiftspeed-siberian'); ?>
                </p>
                <div id="woocommerce_toggle_result" class="swsib-message"></div>
            </div>
        </div>
    ...
    <p>
        ...
        <div class="swsib-notice info">
            <p><?php _e('Enable the WooCommerce integration above to configure and manage subscriptions.', 'swiftspeed-siberian'); ?></p>
        </div>
        <?php else: ?>

        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="swsib-settings-form">
            <?php wp_nonce_field('swsib_woocommerce_nonce', '_wpnonce_swsib_woocommerce'); ?>
            <input type="hidden" name="action" value="swsib_save_woocommerce_settings">
            <input type="hidden" name="tab_id" value="woocommerce">
            <!-- This hidden field contains the integration status -->
            <input type="hidden" name="swsib_options[woocommerce][integration_enabled]" id="integration_enabled_field" value="<?php echo $integration_enabled ? '1' : '0'; ?>">
                ...
                <?php if ($this->are_dependencies_met()): ?>
                    ...
                    <div class="swsib-section">
                        ...
                        <div class="swsib-field">
                            ...
                            <div class="swsib-notice info">
                                <p><?php _e('You have to drag from highest to the lowest, be careful when setting up this part. When a user buys a subscription, the system checks all their active subscriptions, retrieves which has the highest role based on your arrangement, and assigns it to the user. Read our full docs for details.', 'swiftspeed-siberian'); ?></p>
                            </div>
                    ...
                    <div class="swsib-section">
                        ...
                        <div class="swsib-notice info">
                            <p>
                                <strong><?php _e('Important:', 'swiftspeed-siberian'); ?></strong>
                                <?php _e('Configure the popup that will appear on mapped products. In the fields below, enter the text you want to display and either a URL or a shortcode for the popup action. If you enter a URL, a button will be displayed; if you enter a shortcode (e.g. [swsib_login text="Create an App"]) it will show an auto login button and allow your user to reauthenticate with your SiberianCMS.', 'swiftspeed-siberian'); ?>
                            </p>
                        </div>
                        <div class="swsib-field">
