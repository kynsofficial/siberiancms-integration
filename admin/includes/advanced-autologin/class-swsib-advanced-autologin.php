...
class SwiftSpeed_Siberian_Advanced_AutoLogin {
    ...
    public function enqueue_admin_assets($hook) {
        ...
        wp_localize_script(
            ...
            array(
                ...
                'add_button_text' => __('Add Button', 'swiftspeed-siberian'),
    ...
    public function process_form_submission() {
        ...
        // Check nonce
        if (!isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'swsib_advanced_autologin_nonce')) {
        ...
        if (isset($_POST['swsib_options']['advanced_autologin'])) {
            ...
            if (isset($_POST['swsib_options']['advanced_autologin']['buttons']) && 
                ...
                foreach ($_POST['swsib_options']['advanced_autologin']['buttons'] as $button_id => $button_data) {
                    $options['advanced_autologin']['buttons'][$button_id] = array(
                        ...
                        'sync_existing_role' => isset($button_data['sync_existing_role']) ? (bool) $button_data['sync_existing_role'] : false
                    );
                }
            }
        }
        ...
        wp_redirect(admin_url('admin.php?page=swsib-integration&tab_id=advanced_autologin&settings-updated=true'));
        exit;
    }
    ...
    public function ajax_add_button() {
        ...
        $sync_existing_role = isset($_POST['sync_existing_role']) ? (bool) $_POST['sync_existing_role'] : false;
        
        // Get default colors from auto login settings
        $auto_login_options = isset($this->options['auto_login']) ? $this->options['auto_login'] : array();
        $color = isset($auto_login_options['button_color']) ? $auto_login_options['button_color'] : '#3a4b79';
        $text_color = isset($auto_login_options['button_text_color']) ? $auto_login_options['button_text_color'] : '#ffffff';
        ...
        // Return success with button data
    ...
    public function ajax_get_roles() {
        ...
        // Try to get roles directly from DB
        $roles = $this->get_siberian_roles();
        
        if (is_wp_error($roles)) {
            wp_send_json_error(array('message' => $roles->get_error_message()));
        } else {
    ...
    private function get_siberian_roles() {
        // Similar to what's in the autologin class, but simplified
        ...
        try {
            ...
            return new WP_Error('exception', 'Error: ' . $e->getMessage());
        }
    }
    ...
    public function display_settings() {
        ...
        // Get Auto Login settings for reference
        $auto_login_options = isset($this->options['auto_login']) ? $this->options['auto_login'] : array();
        
        // Check if DB is configured
        $is_db_configured = swsib()->is_db_configured();
        
        // Get Siberian roles if DB is configured
        $roles = array();
        $roles_error = '';
        if ($is_db_configured) {
        ...
        <?php else: ?>
            <!-- Settings notice about saving changes -->
            <div class="swsib-notice warning">
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" id="swsib-advanced-autologin-form" class="swsib-settings-form">
                <?php wp_nonce_field('swsib_advanced_autologin_nonce'); ?>
                ...
                <div id="advanced-autologin-settings" style="<?php echo $enabled ? '' : 'display: none;'; ?>">
                    <!-- Explanation of button behavior -->
                    ...
                    <div class="swsib-notice info">
                        ...
                        <p><?php _e('Button appearance and behavior are inherited from the <a href="admin.php?page=swsib-integration&tab_id=auto_login">Auto Login</a> configurations. The only difference is that each button can assign a different role to users.', 'swiftspeed-siberian'); ?></p>
                    </div>
                    ...
                    <div class="swsib-button-creator">
                            ...
                            <?php if (!empty($roles)): ?>
                                <select id="new_button_role">
                                    <?php foreach ($roles as $role): ?>
                                        ...
                                        ?>
                                        <option value="<?php echo esc_attr($role['role_id']); ?>" <?php selected($role['role_id'], 2); ?>><?php echo esc_html($option_text); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            <?php else: ?>
                        ...
                        <!-- New field for sync existing role -->
                        <div class="swsib-field switch-field">
                        ...
                        <!-- Hidden fields for color now using values from Auto Login -->
                        ...
                        <div class="swsib-field">
                            <button type="button" id="add_advanced_button" class="button button-primary"><?php _e('Add Button', 'swiftspeed-siberian'); ?></button>
                        </div>
                    </div>
                        ...
                        <?php else: ?>
                            <?php foreach ($buttons as $button_id => $button): ?>
                                <div class="swsib-button-item" data-id="<?php echo esc_attr($button_id); ?>">
                                    ...
                                    <input type="hidden" name="swsib_options[advanced_autologin][buttons][<?php echo esc_attr($button_id); ?>][sync_existing_role]" value="<?php echo isset($button['sync_existing_role']) ? $button['sync_existing_role'] : '0'; ?>" class="button-sync-role-input" />
                                    ...
                                    <div class="button-info">
                                        ...
                                        <div class="button-sync-role">
                                            <?php if (isset($button['sync_existing_role']) && $button['sync_existing_role']): ?>
                                    ...
                                    <div class="button-edit-form" style="display:none;">
                                        ...
                                        <div class="edit-field switch-field">
                                            ...
                                            <div class="toggle-container">
                                                <label class="switch">
                                                    <input type="checkbox" class="edit-button-sync-role" value="1" <?php checked(isset($button['sync_existing_role']) && $button['sync_existing_role']); ?> />
                                                    <span class="slider round"></span>
                                                </label>
                                                <p class="swsib-field-note swsib-warning-note">
                                                    <strong><?php _e('Warning:', 'swiftspeed-siberian'); ?></strong> 
                    ...
                    <!-- General Button Behavior Section moved below the existing buttons -->
